...
There are 2000 observations in the data set. The dependent variable is the two-level obesity status of the subject. Subjects who are obesity are denoted as "1", and those who are not obesity are denoted as "0". The independent variables include age, gender and systolic blood pressure (SBP).

You can download the data from [Box](https://missouri.box.com/s/s82enrnhdr0nfjr93ym01vns242gu7ow) or [Github](https://raw.githubusercontent.com/zhengyes/logRt/master/rExample.csv).
...
Because the original data are numbers, R will regard all data as numerical data. However, for obesity and gender variables, the numbers just represent their variable levels. In order to tell R about this,  we need to change these two variables to categorical variables. We can use the factor() function to do this.
...
Now we will use the glm (generalized linear regression) function to do the logistic regression. 

We build a model called tutorial.model, the dependent variable on the left side of the tilde (~) is the obesity status, and the independent variables are age, gender and systolic blood pressure (sbp), which are on the right side of the tilde (~), joined by plus sign (+). Note that, gender and obesity are categorical variables. In logistic regression, let p = Probability(subject is obesity), we have, logit(p) = $\beta0$ + $\beta1$ * age + $\beta2$ * gender + $\beta3$ * sbp, the logit function is log odds of obesity which equals to log(\frac{p}{1-p}). logistic regression model is an example of the generalized linear model (GLM).  

Because we just have 0 and 1 for the outcomes, the distritbution of outcome follows a binomial distribution Binomial(n, p) where n is the sample size and p is the probability of being obesity. That is the reason why we have family = "binomial" in the code.
...
Let us first explain what is odds. Odds is the ratio of the probability of success over the probability of the failure. In this tutorial, the odds of obesity equals to probability of being obesity over probability of not being obesity. From the table we can see that, for male the odds of being obesity is 265/690 = 0.3840, for female the odds of being obesity is 381/664 = 0.5738. The odds ratio of obesity, whihc is the ratio of odds for female to the odds for male equals to (381/664)/(265/690)= 1.4943, which means that the odds for female being obesity have 49.43% higher change than the odds for male of being obesity in theis data set before fitting the model.

In the summary result of this model, we can see that all variables are significant with p-values less than 0.05, which means that all model coefficients are significantly different from 0. The coefficient estimation of gender2 equals 0.4910, which means that being female (gender2) increases the log odds of obesity by 0.4910 compared with male (gender1). The log function and exponential function are inverse functions, so we can use the exponential function to get the odds ratio. For here, exp(0.4910) = 1.6340, which means that the odds for female being obesity is 63.4% higher than the odds  of male being obesity. The coefficient estimation of age is 0.0133, which means that one year older in age will increase the log odds of obesity by 0.0133. Exponetial of 0.0133 equals to 1.0134, so one year older leads to 1.34% higher odds for being obesity. The coefficient estimation of sbp is 0.0188, which means that one unit increase in systolic blood pressure leads to 0.0188 increase of log odds of obesity. Exponetial of 0.0188 equals to 1.0190, so one unit increase in SBP leads to 1.9% higher odds for being obesity.
...
The ROC curve is usually a concave curve that towards the upper left corner. The closer the curve is to the corner, the better classification result the model will have. If the curve is a straight line, it means the model has a random calssifier and predicts result no better than by chance. The point denoted in the ROC curve figure below corresponds to the optimal Youden's index value. Note, we use the Youden's index when we think sensitivity and specificity are equally important. In the code, x denotes the fitted values and "class" denoteds the original dependent binary outcome, obesity.
